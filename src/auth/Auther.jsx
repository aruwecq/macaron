...
//               <span>{item.count || 1}x {item.name || item.title}</span>
...
//         <input type="number" placeholder="+7 ___ ___ __ __" value={phone} onChange={e => setPhone(e.target.value)} />

//         <button className="finish-btn" onClick={finishOrder}>Закончить оформление</button>
//       </div>
//     </div>
//   );
// }
...
function Auther() {
  ...
  const finishOrder = async () => {
    ...
    try {
      ...
      // сохраняем заказ в context (если нужно)
      addOrder(order);
